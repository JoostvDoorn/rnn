...
dec_lookup:add(nn.Select(1, -1)) -- To pass along c0
dec_lookup:add(nn.Select(1, -1)) -- To pass along h0
...
for i=1,opt.niter do
   ...
   local decInput = {encLSTM.cell, encOut, decInSeq}
  --  print(encOut)
  --  print(nn.Select(1, -1):forward(encLSTM.cell))
  --  print(nn.Select(1, -1):forward(encOut))
  ...
  --  print(decOut)
   ...
   local grad_c0, grad_h0, _ = unpack(dec:backward(decInput, gradOutput))
   local zeroTensor = torch.Tensor(encOut):zero()
   enc:backward(encInSeq, {grad_c0:select(1, grad_h0:size(1)), grad_h0:select(1, grad_h0:size(1)), zeroTensor})
